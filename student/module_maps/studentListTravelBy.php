					...
					เลือกวิธีการเดินทาง
						...
						<select name="travelby" class="inputboxUpdate">
							...
							<? while($_datTravel = mysqli_fetch_assoc($_resTravel)) { ?>
								<option value="<?=$_datTravel['travel_id']?>" <?=isset($_POST['travelby']) && $_POST['travelby']==$_datTravel['travel_id']?"selected":""?>><?=$_datTravel['travel_description']?></option>
							<? } //end-while ?>
					...
					<? if(mysqli_num_rows($resStudent)>0) { ?>
						<? while($dat = mysqli_fetch_assoc($resStudent)) { ?>
							<tr>
								...
								<td align="center"> <? if(isset($_SESSION['superAdmin'])) { echo "<a href='index.php?option=module_maps/updatemaps&report=studentListTravelBy&travelby=" . $_POST['travelby'] . "&acadyear=" . $acadyear . "&student_id=" . $dat['id'] ."&sex=".$_POST['sex']."'>" . $dat['id'] . "</a>";}
														else {echo $dat['id'] ;} ?> </td>
