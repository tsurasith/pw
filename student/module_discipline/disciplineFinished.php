...
<div id="content">
	...
	<table width="100%"  align="center" border="0" cellspacing="10" cellpadding="0"  class="header">
		<tr> 
			...
			<td><strong><font color="#990000" size="4">งานวินัยนักเรียน</font></strong><br />
				<span class="normal"><font color="#0066FF"><strong>1.1 รายละเอียดพฤติกรรมไม่พึงประสงค์ในแต่ละคดี</strong></font></span>
			</td>
			<td>
			<?php
			...
			?>
				...
				<form action="" method="post" autocomplete="off">
					หมายเลขคดี 
					<? $_resDis = mysqli_query($_connection,"select dis_id from student_disciplinestatus where student_id = '" . $_SESSION['username'] . "' and acadyear = '" . $acadyear . "' and acadsemester = '" . $acadsemester . "'"); ?>
					<select name="dis_id" class="inputboxUpdate">
						<option value=""></option>
						<? while($_datDis = mysqli_fetch_assoc($_resDis)){ ?>
							<option value="<?=$_datDis['dis_id']?>" <?=$_disID==$_datDis['dis_id']?"selected":""?> ><?=$_datDis['dis_id']?></option>
						<? }mysqli_free_result($_resDis); ?>
					</select>
	...
	<? if(mysqli_num_rows($_result)>0){ ?>
			...
			<? } //end if ตรวจสอบว่าสถานะของคดี > 3 หรือไม่ ?>
			
			<? if($_dat['dis_status']>5) { ?>
				...
				<? } //end if ตรวจสอบว่าคดีมีสถานะ > 4 หรือไม่ ?>
...
<?
	...
	function disType($_value)
	{
		switch ($_value)
		{
			...
			default : return "ยังไม่มีการสอบสวน";
		}
	}
