...
<? if(isset($_POST['search']) && $_POST['roomID'] != ""){ ?>
		<form method="post">
		<? $_resRoom = mysqli_query($_connection,"select teacher_id,teacher_id2 from rooms where room_id = '" . $_POST['roomID'] . "' and acadyear = '" . $acadyear . "' and acadsemester = '" . $acadsemester . "'");?>
		...
		<? $_resTeacher2 = mysqli_query($_connection,$_sqlTeacher); ?>
			...
			<tr>
				...
				<td>
					<select name="teacher_id2" class="inputboxUpdate">
						<option value=""></option>
					<? while($_dat = mysqli_fetch_assoc($_resTeacher2)){ ?>
