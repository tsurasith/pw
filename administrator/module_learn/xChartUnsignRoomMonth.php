...
<table width="100%"  align="center" border="0" cellspacing="10" cellpadding="0"  class="header">
    <tr> 
      ...
      <td ><strong><font color="#990000" size="4">Room Tracking</font></strong><br />
        <span class="normal"><font color="#0066FF"><strong>4.4 แสดงจำนวนครั้งที่ไม่ลงชื่อของครูผู้สอน<br/>รายห้อง [รายเดือน]</strong></font></span></td>
      <td >
		<?php
		...
		?>
		ปีการศึกษา<?php  
					echo "<a href=\"index.php?option=module_learn/xChartUnsignRoomMonth&acadyear=" . ($acadyear - 1) . "\"><img src=\"../images/pull_left.gif\" border=\"0\" /></a> " ;
					echo ' <font color=\'blue\'>' .$acadyear . '</font>';
					echo " <a href=\"index.php?option=module_learn/xChartUnsignRoomMonth&acadyear=" . ($acadyear + 1) . "\"><img src=\"../images/pull_right.gif\" border=\"0\" /></a> " ;
				?>
		ภาคเรียนที่   <?php 
					if($acadsemester == 1)
					{
					...
					{
						echo " <a href=\"index.php?option=module_learn/xChartUnsignRoomMonth&acadyear=" . ($acadyear) . "&acadsemester=1 \"> 1</a> , " ;
					}
					if($acadsemester == 2)
					{
					...
					{
						echo " <a href=\"index.php?option=module_learn/xChartUnsignRoomMonth&acadyear=" . ($acadyear) . "&acadsemester=2 \"> 2</a> " ;
					}
				?>
...
<? if(isset($_POST['search']) && ($_POST['month'] == "" || $_POST['roomID'] == "")) { ?>
	<br/><br/><center><font color="#FF0000">กรุณาเลือก ห้องเรียน และ เดือน ที่ต้องการทราบข้อมูลก่อน</font></center>
<? }//end if ?>
...
<? if(isset($_POST['search']) && $_POST['month'] != "" && $_POST['roomID'] != "") { ?>
  <table cellpadding="1" cellspacing="1" border="0" align="center" width="100%">
	<?php
			$sqlStudent = "select period,count(stutus) as late from teachers_learn
								where acadyear = '" . $acadyear . "' and acadsemester = '". $acadsemester . "' and stutus = 'unsign' 
									and month(check_date) = '" . $_POST['month'] . "' and room_id = '" . $_POST['roomID'] . "' 
								group by period
								order by period";
    ...
    <tr > 
		...
		<br/>ประจำเดือน <?=displayMonth($_POST['month'])?> ภาคเรียนที่ <?=$acadsemester?> ปีการศึกษา <?=$acadyear?>
		<br/>
				<?php
					$_strXML = "<?xml version='1.0' encoding='UTF-8' ?>" ;
					$_strXML = $_strXML . "<graph caption='สรุปครั้งที่ไม่ลงชื่อ เดือน" . displayMonth($_POST['month']) . "' xAxisName='คาบเรียน' yAxisName='Units' decimalPrecision='0' formatNumberScale='0'>";
