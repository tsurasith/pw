...
<div id="content">
	...
	<? if($_check_data>0) { ?>
		<div align="center">
			<table  align="center" cellspacing="1" class="admintable">
				...
				<tr>
					<td>
						...
						<form method="post" action="index.php?option=module_learn/insertStudentCheckSinglePeriod">
			...
			$sql = "
					SELECT id, prefix , firstname , lastname 
					FROM 
						students 
					WHERE 
						xLevel =  '" . $xlevel . "' AND xYearth = '" . $xyear . "' and 
						room = '" . $room_id  .  "' and xedbe = '" .$acadyear . "'  and 
						studstatus = '1' 
		...
		//	echo $sql;
			...
			while($data = mysqli_fetch_array($result))
			{
				...
				echo "<td align='center'>
							...
							<input type='hidden' name='student_name[$j]' value='$data[1]$data[2] $data[3]' />
				...
				echo "<td align='center'><input type='radio' name='check[$j]' value='lightblue'   onclick='check(this.name,this.value)' ></td>";
				echo "<td align='center'><input type='radio' name='check[$j]' value='orange'  onclick='check(this.name,this.value)' ></td>";	
		...
		?>
													<tr bgcolor="#FFFFFF"> 
													<td colspan="9" align="center"> <input type="hidden" name="count" value="<?php  echo $j; ?>"/>
														<table>
															<tr>
																<td>
