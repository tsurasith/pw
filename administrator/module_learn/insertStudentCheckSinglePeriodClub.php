...
if(isset($_POST['save']) && $_POST['date'] != ""){
	...
	$_student_disappear .= " รหัสชุมนุม/กิจกรรม " . $_POST['club_code'] . ' ' . $_POST['club_name'] . " คาบ " . $_POST['period'] . " วันที่ " . displayDateChart($_POST['date']). "";
	for($i =0 ;$i < $_POST['count'] ;$i++)
	{
		$row_id = '';
		if($i < 10) { 
			$row_id = $_POST['date'] . '-00' . $_POST['period'] . '-' . $_POST['student_id'][$i]; 
		}
		else { 
			$row_id = $_POST['date'] . '-00' . $_POST['period'] . '-' . $_POST['student_id'][$i]; 
		}
		...
		if(timecheck_id($_POST['check'][$i]) == "05"){
		{
			...
			$_discipline_details = "หนีเรียน รหัสชุมนุม/กิจกรรม " . $_POST['club_code'] . ' ' . $_POST['club_name'] . " คาบ " . $_POST['period'] . " วันที่ " . displayDateChart($_POST['date']) ;
	...
	*/
	...
	$_student_learn_task_processing = true;
				...
				if($_student_learn_proccesing && $_student_learn_task_processing){
					...
					$_verifySQL = "
									...
									where 
										...
										club_code   = '" . $_POST['club_code'] . "' and
										period      = '" . $_POST['period'] . "'  and
										teacher_id  = '" . $_POST['teacher_id'] . "' 
									group by club_code ";
					$_text = "ผลการบันทึกการเข้าเรียน";
					$_text .= " รหัสชุมนุม/กิจกรรม " . $_POST['club_code'] . ' ' . $_POST['club_name'] . " คาบเรียนที่ " . $_POST['period'];
					$_text .= " " . reportHeader($_POST['date']);
					...
					if($result){
						...
						$_event_details = "";
						$_event_details .= "บันทึกการเข้าเรียน " . $_POST['teacher_id'] ." รหัสวิชา " . $_POST['club_code'] . " คาบเรียนที่ " . $_POST['period'];
