					...
					<table align="center" >
						...
						<tr>
							...
							<? while($_dat = mysqli_fetch_assoc($_result)) { ?>
								<? $_catXML .= "<category name='" . ($_dat['occup']==""?"ไม่ระบุ":displayOccupation($_connection,$_dat['occup'])) . "' hoverText=''/>"; ?>
								<? $_color = getFCColor(); ?>
								<? $_setA .= "<set value='" . $_dat['male'] . "' color = '" . $_color . "' />"; ?>
								<? $_setB .= "<set value='" . $_dat['female'] . "' color = '" . $_color . "' />"; ?>
								<? $_xmlPie .= "<set value='" . ($_dat['male']+$_dat['female']) . "' name='" .($_dat['occup']==""?"ไม่ระบุ":displayOccupation($_connection,$_dat['occup'])). "' color='" . $_color . "'/>"; ?>
								<? $_m += $_dat['male']; $_f +=$_dat['female']; $_t += $_dat['total']; ?>
								<tr>
									<td align="left" style="padding-left:15px;"><?=$_dat['occup']==""?"ไม่ระบุ":displayOccupation($_connection,$_dat['occup'])?></td>
