...
<table width="100%"  align="center" border="0" cellspacing="10" cellpadding="0"  class="header">
    <tr> 
      ...
      <td><strong><font color="#990000" size="4">ระบบรายงาน/สถิติ</font></strong><br />
        <span class="normal"><font color="#0066FF"><strong>1.5.5 วิเคราะห์วิชาที่ชอบ เพศ และการเข้าชั้นเรียน</strong></font></span></td>
      <td width="55%">
		<?php
		...
		?>
		ปีการศึกษา<?php  
					echo "<a href=\"index.php?option=module_reports/ReportLearnLike&acadyear=" . ($acadyear - 1) . "\"><img src=\"../images/pull_left.gif\" border=\"0\" /></a> " ;
					echo ' <font color=\'blue\'>' .$acadyear . '</font>';
					echo " <a href=\"index.php?option=module_reports/ReportLearnLike&acadyear=" . ($acadyear + 1) . "\"><img src=\"../images/pull_right.gif\" border=\"0\" /></a> " ;
				?>
		 ภาคเรียนที่   <? if($acadsemester == 1) { echo "<font color='blue'>1</font> , "; }
					else { echo " <a href=\"index.php?option=module_reports/ReportLearnLike&acadyear=" . ($acadyear) . "&acadsemester=1 \"> 1</a> , " ; }
					if($acadsemester == 2) { echo "<font color='blue'>2</font>"; }
					else {
						echo " <a href=\"index.php?option=module_reports/ReportLearnLike&acadyear=" . ($acadyear) . "&acadsemester=2 \"> 2</a> " ;
					}
				?>
		<br/>
...
<? if(isset($_POST['search']) && $_POST['timecheck'] != ""){ ?>
	<? $_sql = "select subject_like,count(distinct id) as 'student',
	...
	<? $_sql .=	" group by subject_like order by 2 desc"; ?>
		...
		<table class="admintable" width="100%" align="center" >
			<tr>
				<th colspan="2" align="center">
					...
					<br/>
					วิเคราะห์วิชาที่ชอบ เพศ <br/>และการเข้าชั้นเรียน
			...
			<tr>
				<td colspan="2" align="center">
					<table align="center" >
						<tr>
							<td class="key" rowspan="2" align="center" width="180px">วิชา</td>
							...
							<td class="key" rowspan="2" align="center" width="70px">% แต่ละ วิชา</td>
						</tr>
						<tr>
							...
							<? while($_dat = mysql_fetch_assoc($_result)) { ?>
								<? if($_dat['sum']>0){ ?>
									<? $_catXML .= "<category name='" . ($_dat['subject_like']==""?"ไม่ระบุ":$_dat['subject_like']) . "' hoverText=''/>"; ?>
									<? $_color = getFCColor(); ?>
									<? $_setA .= "<set value='" . $_dat['male'] . "' color = '" . $_color . "' />"; ?>
									<? $_setB .= "<set value='" . $_dat['female'] . "' color = '" . $_color . "' />"; ?>
									<? $_xmlPie .= "<set value='" . (100*$_dat['sum']/$_dat['total']) . "' name='" .($_dat['subject_like']==""?"ไม่ระบุ":$_dat['subject_like']). "' color='" . $_color . "'/>"; ?>
								<? } //end ตรวจสอบข้อมูลว่างไม่ให้แสดงที่แผนภูมิ ?>
								<? $_m += $_dat['male']; $_f +=$_dat['female']; $_x += $_dat['sum'];
									$_s += $_dat['student']; $_t += $_dat['total']; ?>
								<tr>
									<td align="left" style="padding-left:15px;"><?=$_dat['subject_like']==""?"ไม่ระบุ":$_dat['subject_like']?></td>
