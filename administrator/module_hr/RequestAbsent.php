	...
	<?php
		$_sql_initial = "
				...
				LEFT JOIN departments d ON
					(
						s.department_id = d.department_id AND s.acadyear = d.acadyear AND s.acadsemester = d.acadsemester
					)
				LEFT JOIN department_subtypes t ON
					(
						s.sub_department_id = t.department_subtype_id AND s.acadyear = t.acadyear AND s.acadsemester = t.acadsemester AND s.department_id = t.department_id
					)
				WHERE
					s.staff_id = '" . $_staff_id . "' and
		...
		<form method="post" name="myform" enctype="multipart/form-data" autocomplete="off">
			<table class="admintable">
				...
				<tr>
					...
					<td>
						<input type="hidden" name="department_id" value="<?=$_dat['department_id']?>"/>
						<input class="noborder2" type="text" size="40" readonly="true" name="department_name" value="<?=$_dat['department_name']?>"/>
					</td>
				</tr>
				<tr>
					<td align="right">งานที่รับผิดชอบ :</td>
					<td>
						<input type="hidden" name="sub_department_id" value="<?=$_dat['sub_department_id']?>"/>
						<input class="noborder2" type="text" size="50" readonly="true" name="sub_department_name" value="<?=$_dat['department_subtype_name']?>"/>
					</td>
				</tr>
				<tr>
					<td align="right">ขออนุญาต :</td>
					<td>
