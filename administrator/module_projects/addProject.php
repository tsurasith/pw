...
<?php
	...
	if(isset($_POST['action']) && $_POST['action'] == "save"){
		...
		if($_res){
			$_processing_text = "บันทึกข้อมูลโครงการ " . trim($_POST['project_name']) . " เรียบร้อยแล้ว";
			$_processing_result = true;

			// line message here
			$_text .= "โครงการ: ";
			$_text .= trim($_POST['project_name']) . " ถูกเพิ่มเข้าในระบบแล้ว";
	...
	$_sql_project = "select * from projects where project_id = '" . $_project_id . "'";
	$_res_project = mysqli_query($_connection,$_sql_project);

	if(mysqli_num_rows($_res_project)>0){
		...
		<table width="100%"  align="center" border="0" cellspacing="10" cellpadding="0"  class="header">
			<tr> 
				...
				<td >
					...
					<span class="normal"><font color="#0066FF"><strong>1.1 บันทึกข้อมูลโครงการ</strong></font></span>
				</td>
				<td >
					ปีการศึกษา <?=$acadyear?>
				...
				<tr>
					...
					<td>
						<input  type="text" id="project_number" name="project_number" class="noborder2" 
								size="20" maxlength="20" value="<?=isset($_POST['action'])?$_project['project_number']:""?>" />
				...
				<tr>
					...
					<td>
						<input  type="text" id="project_name" name="project_name" class="noborder2" s
								...
								value="<?=isset($_POST['action'])?$_project['project_name']:""?>" />
						<font color="#FF0000">*</font>
					</td>
				</tr>
				<tr>
					...
					<td>
						<textarea id="project_details" name="project_details" cols="50" rows="6" 
								  class="inputboxUpdate" ><?=isset($_POST['action'])?$_project['project_details']:""?></textarea>
					</td>
				</tr>
				<tr>
					...
					<td>
						<input  type="text" name="start_date" id="start_date" size="10" 
								...
								value="<?=isset($_POST['action'])?$_project['start_date']:""?>" class="noborder2"  />
						<font color="#FF0000">*</font>
					</td>
				</tr>
				<tr>
					...
					<td>
						<input  type="text" name="finish_date" id="finish_date" size="10" 
								onClick="showCalendar(this.id)" 
								value="<?=isset($_POST['action'])?$_project['finish_date']:""?>" class="noborder2"  />
						<font color="#FF0000">*</font>
					</td>
				</tr>
				<tr>
					...
					<td>
						...
						<select id="income_id" name="income_id" class="inputboxUpdate">
							...
							<? while($_dat = mysqli_fetch_assoc($_res_income)) { ?> 
								<option value="<?=$_dat['income_id']?>" <?=isset($_POST['action'])&&$_project['income_id']==$_dat['income_id']?"selected":""?> >
									<?=$_dat['income_name']?>
								</option>
							<? } ?>
						</select>
				...
				<tr>
					...
					<td>
						<input  type="text" id="approve_budget" name="approve_budget" 
								size="15" maxlength="13" value="<?=isset($_POST['action'])?$_project['approve_budget']:""?>" 
								onkeypress="return isNumberKey2(event)" class="noborder2"  /> 
								บาท <font color="#FF0000">*</font>
					</td>
				</tr>
				<tr>
					...
					<td>
						...
						<select id="department_id" name="department_id" class="inputboxUpdate">
							<option value=""></option>
							<? while($_dat = mysqli_fetch_assoc($_res_income)) { ?> 
								<option value="<?=$_dat['department_id']?>" <?=isset($_POST['action'])&&$_project['department_id']==$_dat['department_id']?"selected":""?> >
						...
						?>
							...
							<select id="project_owner" name="project_owner" class="inputboxUpdate">
								...
								<?php
									...
									{
										if(isset($_POST['action']) && $_project['project_owner'] == $dat['staff_id']){
											$_select = "selected";
										}else{
											$_select = "";
										}
					...
					<td>
						...
						<select id="project_status" name="project_status" class="inputboxUpdate">
							<? while($_dat = mysqli_fetch_assoc($_res_status)) { ?> 
								<option value="<?=$_dat['status_id']?>" <?=isset($_POST['action'])&& $_project['project_status']==$_dat['status_id']?"selected":""?> >
									<?=$_dat['status_name']?>
								</option>
							<? } ?>
						</select>
					...
					<td>
						<?php
							...
							if(isset($_POST['action'])){
								...
								$_button      .= "onclick=\"location.href='index.php?option=module_projects/addProject'\" ";
								$_button      .= " class=\"button\" />";
							}else{
								$_action = "save";
							}
						?>
						<input type="hidden" name="used_budget"  value="<?=isset($_POST['action'])?$_project['used_budget']:"0"?>" />
