...
<table width="100%"  align="center" border="0" cellspacing="10" cellpadding="0"  class="header">
	...
	<span class="normal"><font color="#0066FF"><strong>2. รายละเอียดโครงการฉบับเต็ม</strong></font></span></td>
      <td >
		<?php
		...
		?>
		ปีการศึกษา<?php  
					echo "<a href=\"index.php?option=module_projects/projectFull&acadyear=" . ($acadyear - 1) . "\"><img src=\"../images/pull_left.gif\" border=\"0\" /></a> " ;
					echo ' <font color=\'blue\'>' .$acadyear . '</font>';
					echo " <a href=\"index.php?option=module_projects/projectFull&acadyear=" . ($acadyear + 1) . "\"><img src=\"../images/pull_right.gif\" border=\"0\" /></a> " ;
				?>
		ภาคเรียนที่   <?php 
					if($acadsemester == 1) { echo "<font color='blue'>1</font> , "; }
					else {
						echo " <a href=\"index.php?option=module_projects/projectFull&acadyear=" . ($acadyear) . "&acadsemester=1 \"> 1</a> , " ;
					}
					if($acadsemester == 2) { echo "<font color='blue'>2</font>"; }
					else {
						echo " <a href=\"index.php?option=module_projects/projectFull&acadyear=" . ($acadyear) . "&acadsemester=2 \"> 2</a> " ;
					}

					$_pID = "";
					$_pID = isset($_POST['p_id'])?$_POST['p_id']:"";
					if(isset($_REQUEST['p_id'])) $_pID = $_REQUEST['p_id'];
				?>
			...
			<form method="post" autocomplete="off">
				...
				<select name="p_id" class="inputboxUpdate">
					...
					<? while($_dat = mysqli_fetch_assoc($_res)) { ?>
						<option value="<?=$_dat['project_id']?>" <?=$_pID==$_dat['project_id']?"selected":""?>><?=strlen(trim($_dat['project_name']))>90?(substr($_dat['project_name'],0,90) . "..."):$_dat['project_name']?></option>
					<? }//end while ?>
