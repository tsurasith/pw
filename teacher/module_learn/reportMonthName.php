...
<table width="100%"  align="center" border="0" cellspacing="10" cellpadding="0"  class="header">
    <tr> 
      ...
      <td ><strong><font color="#990000" size="4">Room Tracking</font></strong><br />
        <span class="normal"><font color="#0066FF"><strong>1.1 รายงานการเข้าเรียนสายของนักเรียน<br/>แบบรายชื่อ [รายเดือน]</strong></font></span></td>
       <td >
		<?php
			$_error = 1;
		...
		?>
		ปีการศึกษา<?php  
					echo "<a href=\"index.php?option=module_learn/reportMonthName&acadyear=" . ($acadyear - 1) . "\"><img src=\"../images/pull_left.gif\" border=\"0\" /></a> " ;
					echo ' <font color=\'blue\'>' .$acadyear . '</font>';
					echo " <a href=\"index.php?option=module_learn/reportMonthName&acadyear=" . ($acadyear + 1) . "\"><img src=\"../images/pull_right.gif\" border=\"0\" /></a> " ;
				?>
		ภาคเรียนที่   <?php 
					if($acadsemester == 1)
					{
					...
					{
						echo " <a href=\"index.php?option=module_learn/reportMonthName&acadyear=" . ($acadyear) . "&acadsemester=1 \"> 1</a> , " ;
					}
					if($acadsemester == 2)
					{
					...
					{
						echo " <a href=\"index.php?option=module_learn/reportMonthName&acadyear=" . ($acadyear) . "&acadsemester=2 \"> 2</a> " ;
					}
				?>
  ...
  <table class="admintable"  cellpadding="1" cellspacing="1" border="0" align="center" width="100%" >
	...
	<?php
		...
		{
			echo "<td align='center'><font color='red'><br/>กรุณาเลือกระดับชั้น และ เดือนที่ต้องการทราบข้อมูลก่อน !</font></td></tr>";
		}
		else if(isset($_POST['search']) && $_POST['month'] != "")
		{
			...
			if(isset($_POST['roomID']) && $_POST['roomID'] != "all")
			{
				$sqlStudent = "select students.id,students.prefix,students.firstname,students.lastname,p_village,students.xyearth,students.xlevel,students.room,students.studstatus ,
								...
								where month(check_date) = '" . $_POST['month'] . "' and timecheck_id = '02'
				...
				$sqlStudent .= " group by id order by late desc,room,sex,id";
			}
			else
			{
				$sqlStudent = "select students.id,students.prefix,students.firstname,students.lastname,p_village,students.xyearth,students.xlevel,students.room,students.studstatus ,
								  count(timecheck_id) as late
								from students  left outer join student_learn on students.id = student_learn.student_id
								where month(check_date) = '" . $_POST['month'] . "' and xedbe = '" . $acadyear . "' and timecheck_id = '02'
									  and acadyear = '" . $acadyear . "' and acadsemester = '" . $acadsemester . "' ";
				if(isset($_POST['studstatus']) && $_POST['studstatus']=="1,2") $sqlStudent .= "and studstatus in (1,2) ";
				$sqlStudent .= " group by id order by late desc,xlevel,xyearth,room,sex,id";			
			}
