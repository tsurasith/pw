...
<table width="100%"  align="center" border="0" cellspacing="10" cellpadding="0"  class="header">
    <tr> 
      ...
      <td><strong><font color="#990000" size="4">ระบบรายงาน/สถิติ</font></strong><br />
        <span class="normal"><font color="#0066FF"><strong>1.6.5 สารสนเทศนักเรียนตามสถานภาพความขาดแคลน</strong></font></span></td>
      <td >
		<?php
		...
		?>
		ปีการศึกษา<?php  
					echo "<a href=\"index.php?option=module_reports/ReportHistoryAbsent&acadyear=" . ($acadyear - 1) . "\"><img src=\"../images/pull_left.gif\" border=\"0\" /></a> " ;
					echo ' <font color=\'blue\'>' .$acadyear . '</font>';
					echo " <a href=\"index.php?option=module_reports/ReportHistoryAbsent&acadyear=" . ($acadyear + 1) . "\"><img src=\"../images/pull_right.gif\" border=\"0\" /></a> " ;
				?>
		<br/>
...
<? if(isset($_POST['search']) && $_POST['roomID'] != ""){ ?>
	...
	<? $_sql .=	" group by studabsent order by count(*) desc"; ?>
		...
		<table class="admintable" width="100%" align="center" >
			<tr>
				<th colspan="2" align="center">
					...
					<br/>
					สารสนเทศนักเรียนตามสถานภาพความขาดแคลน 
					...
					<table align="center" >
						...
						<tr>
							...
							<? while($_dat = mysqli_fetch_assoc($_result)) { ?>
								<? $_catXML .= "<category name='" . ($_dat['studabsent']==""?"ไม่ระบุ":displayStudAbsent($_dat['studabsent'])) . "' hoverText=''/>"; ?>
								<? $_color = getFCColor(); ?>
								<? $_setA .= "<set value='" . $_dat['male'] . "' color = '" . $_color . "' />"; ?>
								<? $_setB .= "<set value='" . $_dat['female'] . "' color = '" . $_color . "' />"; ?>
								<? $_xmlPie .= "<set value='" . ($_dat['male']+$_dat['female']) . "' name='" .($_dat['studabsent']==""?"ไม่ระบุ":displayStudAbsent($_dat['studabsent'])). "' color='" . $_color . "'/>"; ?>
								<? $_m += $_dat['male']; $_f +=$_dat['female']; $_t += $_dat['total']; ?>
								<tr>
									<td align="left" style="padding-left:15px;"><?=$_dat['studabsent']==""?"ไม่ระบุ":displayStudAbsent($_dat['studabsent'])?></td>
