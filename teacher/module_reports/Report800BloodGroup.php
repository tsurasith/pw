...
<table width="100%"  align="center" border="0" cellspacing="10" cellpadding="0"  class="header">
    <tr> 
      ...
      <td><strong><font color="#990000" size="4">ระบบรายงาน/สถิติ</font></strong><br />
        <span class="normal"><font color="#0066FF"><strong>1.4.1 วิเคราะห์หมู่โลหิต เพศ <br/>และการเข้าร่วมกิจกรรมหน้าเสาธง</strong></font></span></td>
      <td width="55%">
		<?php
		...
		?>
		ปีการศึกษา<?php  
					echo "<a href=\"index.php?option=module_reports/Report800BloodGroup&acadyear=" . ($acadyear - 1) . "\"><img src=\"../images/pull_left.gif\" border=\"0\" /></a> " ;
					echo ' <font color=\'blue\'>' .$acadyear . '</font>';
					echo " <a href=\"index.php?option=module_reports/Report800BloodGroup&acadyear=" . ($acadyear + 1) . "\"><img src=\"../images/pull_right.gif\" border=\"0\" /></a> " ;
				?>
		 ภาคเรียนที่   <? if($acadsemester == 1) { echo "<font color='blue'>1</font> , "; }
					else { echo " <a href=\"index.php?option=module_reports/Report800BloodGroup&acadyear=" . ($acadyear) . "&acadsemester=1 \"> 1</a> , " ; }
					if($acadsemester == 2) { echo "<font color='blue'>2</font>"; }
					else {
						echo " <a href=\"index.php?option=module_reports/Report800BloodGroup&acadyear=" . ($acadyear) . "&acadsemester=2 \"> 2</a> " ;
					}
				?>
		<br/>
		<font color="#000000" size="2"  >
		<form method="post">
			การร่วมกิจกรรมหน้าเสาธง 
...
<? if(isset($_POST['search']) && $_POST['timecheck'] =="") { ?>
	<? echo "<br/><br/><center><font color='red'>กรุณาเลือกการมาเข้าร่วมกิจกรรมหน้าเสาธงก่อน !</font></center>";} ?>	
<? if(isset($_POST['search']) && $_POST['timecheck'] != ""){ ?>
	<? $_sql = "select blood_group,count(distinct id) as 'student',
	...
	<? $_sql .=	" group by blood_group order by 1 desc"; ?>
		...
		<table class="admintable" width="100%" align="center" >
			<tr>
				<th colspan="2" align="center">
					...
					<br/>
					วิเคราะห์หมู่โลหิต เพศ <br/>และการเข้าร่วมกิจกรรมหน้าเสาธง
			...
			<tr>
				<td colspan="2" align="center">
					<table align="center" >
						<tr>
							<td class="key" rowspan="2" align="center" width="80px">หมู่โลหิต</td>
							...
							<td class="key" rowspan="2" align="center" width="60px">% แต่ละหมู่โลหิต</td>
						</tr>
						<tr>
							...
							<? while($_dat = mysql_fetch_assoc($_result)) { ?>
								<? $_catXML .= "<category name='" . ($_dat['blood_group']==""?"ไม่ระบุ":$_dat['blood_group']) . "' hoverText=''/>"; ?>
								<? $_color = getFCColor(); ?>
								<? $_setA .= "<set value='" . $_dat['male'] . "' color = '" . $_color . "' />"; ?>
								<? $_setB .= "<set value='" . $_dat['female'] . "' color = '" . $_color . "' />"; ?>
								<? $_xmlPie .= "<set value='" . (100*$_dat['sum']/$_dat['total']) . "' name='" .($_dat['blood_group']==""?"ไม่ระบุ":$_dat['blood_group']). "' color='" . $_color . "'/>"; ?>
								<? $_m += $_dat['male']; $_f +=$_dat['female']; $_x += $_dat['sum'];
									$_s += $_dat['student']; $_t += $_dat['total']; ?>
								<tr>
									<td align="left" style="padding-left:15px;"><?=$_dat['blood_group']==""?"ไม่ระบุ":$_dat['blood_group']?></td>
									<td align="right" style="padding-right:20px;"><?=number_format($_dat['student'],0,'',',')?></td>
